...
def instfabric(ver,loader,url='meta.fabricmc.net',d='.minecraft',v1=None):#ver为要安装fabric的mc版本,安装完后记得补全文件
    ...
    if rs.status_code==200:
        v=rs.json()['id']
        p=pj(d,'versions',v1 or v)
        #if os.path.exists(p):raise RuntimeError('已经安装过了')
        mkdir(p)
        with open(pj(p,v+'.json'),'wb') as f:
            f.write(rs.content)
        return v
...
def optilist(url='https://optifine.net/downloads'):
    html=req.get(url,verify=False,timeout=100).text
...
def getforgev(ver,url='files.minecraftforge.net'):
    ...
    try:return rt
    except:RuntimeError('没有支持该版本的forge!')
...
def instforge_part2(lzp,dc,instp,java,d='.minecraft'):#part2
